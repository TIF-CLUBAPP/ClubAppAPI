    ...
    <div className="min-h-screen w-full bg-slate-950 flex items-center justify-center p-4 selection:bg-emerald-500 selection:text-slate-950">
      ...
      >
        ...
        <motion.div
          ...
          className="inline-flex p-5 rounded-2xl bg-rose-500/10 text-rose-400 border border-rose-500/20 mb-6"
        >
          <AlertTriangle size={40} />
        </motion.div>

        {/* Título */}
        <h1 className="text-3xl font-extrabold tracking-tight text-white mb-3">Pago Fallido</h1>

        {/* Mensaje */}
        <p className="text-slate-400 text-base leading-relaxed mb-8 max-w-xs mx-auto">
          No pudimos procesar el pago de tu cuota. Podés intentarlo nuevamente
          desde Mis Cuotas. Si el problema persiste, contactá a la administración del club.
        </p>
        ...
        <div className="mt-8 pt-6 border-t border-slate-800">
          <p className="text-[11px] text-slate-500 font-mono">
            {cuotaId ? `Cuota N° ${cuotaId} · Pago rechazado` : 'El pago no pudo completarse'}
          </p>
        </div>
      </motion.div>
    </div>
